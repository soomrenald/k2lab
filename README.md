...
The first regional-prompting backend is available through **Use enabled regional prompts**. It independently encodes every non-empty regional prompt, maps each half-open pixel box to fractional latent and 16-pixel image-token masks, evaluates a cropped denoiser pass for that region, and blends it with the global prediction. **Regional strength** controls the regional/global blend weight and **Region feather** applies an inward smoothstep taper so influence reaches zero at the box edge. Region negative prompts, the enable flag, strength, feather, boxes, and compiled backend summary are persisted in project/image metadata. This `comfy-latent-area-v1` backend establishes working spatial conditioning; native Krea single-stream text/image attention-bias routing and regional LoRA delta gating remain later milestones.
