...
The separate **Image editing** workspace sits between the generation canvas and face refinement. Load a PNG, JPEG, or WebP image, draw one or more rectangular edit regions, and give each region its own prompt. When the image contains embedded `k2lab_project` metadata—or has an exact `<image-stem>.k2lab.json` sidecar—the editor restores the original global and regional prompts, boxes, prompt emphases, LoRAs, projector settings, sampler, step count, and seed. The restored seed is fixed by default. **Reference layout** and **Edit targets** are separate synchronized GUI layers, so original conditioning boxes can be inspected or adjusted without turning them into denoise masks.

Image editing VAE-encodes the complete source, pads only the right and bottom edges to Krea's 16-pixel alignment without rescaling, and runs one img2img pass through the same full-canvas attention and routed-LoRA stack as generation. Original regional subject clauses retain identity ownership while edit clauses add a non-owning regional delta, allowing edited pixels to attend to both the requested change and the surrounding/reference identity. The original global scene prompt remains visible and round-trips with the project, but is deliberately excluded from edit conditioning because source scene text can directly conflict with replacement and removal instructions. Whole-image edits likewise condition on only the new edit instruction. The union of edit boxes, with a wider latent transition collar, is passed to ComfyUI as the denoise mask; latent samples outside it are clamped back to the source at every step. After decoding, source pixels are restored exactly outside the narrower composite feather. This protection applies even when the edit instruction field is non-empty. Whole-image denoising occurs only when **Edit entire image** is explicitly enabled and can still cause broad identity or scene drift, so boxed edits are strongly recommended. The defaults use low denoise, full reference retention, a fixed original seed, a 64 px latent transition, and a 48 px final feather to reduce seams and identity drift.
