...
Each loaded LoRA has a saved model-strength control and an **Inspect selected LoRA…** action. Inspection runs in the loaded worker, reports exact Krea transformer target coverage, and catches files that only appear to load because their namespaces do not match. K2 Lab aligns the AI Toolkit `diffusion_model.blocks…` and `diffusion_model.txtfusion…` conventions used by Krea LoRAs with the namespace exposed by the active ComfyUI worker. Compatible LoRAs use unfused forward adapters, so base FP8 weights are never rewritten. A Global route enables all text and image lanes. A **Standard regional** route is image-token-only: its delta is enabled only on the 16×16 image-token cells intersecting the assigned half-open pixel boxes. Text-fusion targets and attention key/value targets are omitted for this route because their outputs are shared with every image query and therefore cannot be spatially bounded by a token-output mask. Query, gate, attention-output, and MLP deltas remain available and are masked to the assigned image tokens. One LoRA may use the union of several named boxes, several LoRAs may share a box, and global and regional adapters can run together through one composite layer hook. Boundary cells are weighted by their actual pixel overlap. Compatibility coverage, locality-skipped targets, route coverage, and measured delta RMS are recorded in PNG metadata.
...
The prompt-attention field and LoRA gate intentionally have different semantics. Prompt placement can feather outside a box so the model composes a coherent scene. Standard regional LoRA parameter deltas use only the strict box mask and do not use that falloff. **Character identity (face)** retains its dedicated regional trigger anchor and established regional-clause text behavior because identity adapters depend on that surrounding semantic context; the separate face-detail pass remains crop-local.
