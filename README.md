...
**Inside boost**, **Outside penalty**, and **Spatial falloff** separately control the field. The overlap-competition and late-step relaxation checkboxes can be disabled for comparison runs. The GUI can preview the compiled prompt, and the prompt, effective roles, token spans, pixel boxes, settings, and attention-call count are stored in image metadata. The `krea-unified-spatial-attention-v2` backend still uses one global denoising pass; its boxes are stronger placement targets, not hard masks. Regional LoRA delta gating remains the next model-control milestone. Regional negative text remains persisted, but Turbo's CFG-free path has no negative branch to route; localized negative conditioning is reserved for Raw/CFG support.
