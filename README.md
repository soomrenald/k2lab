...
Each loaded LoRA has a saved model-strength control and an **Inspect selected LoRA…** action. Inspection runs in the loaded worker, reports exact Krea transformer target coverage, and catches files that only appear to load because their namespaces do not match. K2 Lab normalizes the AI Toolkit `diffusion_model.blocks…` and `diffusion_model.txtfusion…` conventions used by Krea LoRAs before handing adapters to ComfyUI. LoRAs assigned to **Global** are applied to a temporary model patcher for that generation, support multiple additive adapters, and are recorded with strengths and target counts in PNG metadata. Region-scoped LoRAs are explicitly reported as pending rather than silently applied globally; pixel-space LoRA delta gating is the next model-control milestone.
