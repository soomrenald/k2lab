...
The separate **Image editing** workspace sits between the generation canvas and face refinement. Load a PNG, JPEG, or WebP image, draw one or more rectangular edit regions, and give each region its own prompt. When the image contains embedded `k2lab_project` metadata—or has an exact `<image-stem>.k2lab.json` sidecar—the editor restores the original global and regional prompts, boxes, prompt emphases, LoRA files, projector settings, sampler, step count, and seed. Generation settings are inspection-only in this workspace: source prompts, regions, emphases, projector settings, and LoRA routes are never sent to an image-edit worker. Restored LoRA files remain available until explicitly assigned under **Edit targets**. The restored seed is fixed by default. **Source layout** and **Edit targets** are separate synchronized GUI layers, so original boxes can be inspected without becoming conditioning or denoise masks.

Image editing VAE-encodes the complete source, pads only the right and bottom edges to Krea's 16-pixel alignment without rescaling, and runs one img2img pass conditioned only by the edit instruction, edit-target clauses, and LoRAs assigned in Edit mode. The source latent supplies visual continuity without reasserting obsolete generation descriptions such as a color being replaced. The union of edit boxes, with a wider latent transition collar, is passed to ComfyUI as the denoise mask; latent samples outside it are clamped back to the source at every step. After decoding, source pixels are restored exactly outside the narrower composite feather. This protection applies even when the edit instruction field is non-empty. Whole-image denoising occurs only when **Edit entire image** is explicitly enabled and can still cause broad identity or scene drift, so boxed edits are strongly recommended.
