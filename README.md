...
Image editing VAE-encodes the complete source, pads only the right and bottom edges to Krea's 16-pixel alignment without rescaling, and runs one img2img pass through the same full-canvas attention and routed-LoRA stack as generation. Original subject clauses retain identity ownership while edit clauses add a non-owning regional delta, allowing edited pixels to attend to both the requested change and the surrounding/reference identity. The union of edit boxes, with a wider latent transition collar, is passed to ComfyUI as the denoise mask; latent samples outside it are clamped back to the source at every step. After decoding, source pixels are restored exactly outside the narrower composite feather. This protection applies even when the edit instruction field is non-empty. Whole-image denoising occurs only when **Edit entire image** is explicitly enabled. The defaults use low denoise, full reference retention, a fixed original seed, a 64 px latent transition, and a 48 px final feather to reduce seams and identity drift.
