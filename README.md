...
Each loaded LoRA has a saved model-strength control and an **Inspect selected LoRA…** action. Inspection runs in the loaded worker, reports exact Krea transformer target coverage, and catches files that only appear to load because their namespaces do not match. K2 Lab aligns the AI Toolkit `diffusion_model.blocks…` and `diffusion_model.txtfusion…` conventions used by Krea LoRAs with the namespace exposed by the active ComfyUI worker. Compatible LoRAs use unfused forward adapters, so base FP8 weights are never rewritten. A Global route enables all text and image lanes. A regional route enables only the regional clause's text-token span and the 16×16 image-token cells intersecting its assigned half-open pixel box; its direct delta is zero on all other lanes. One LoRA may use the union of several named boxes, several LoRAs may share a box, and global and regional adapters can run together through one composite layer hook. Boundary cells are weighted by their actual pixel overlap. Strength, all 256 target matches, route coverage, and measured text/image delta RMS are recorded in PNG metadata; the reported outside-gate delta is exactly zero by construction.

**Strict regional LoRA isolation** is on by default. Krea mixes all text and image tokens in one transformer stream, so a directly gated token can otherwise carry a regional LoRA's effect to an unassigned subject through later attention layers. When at least one active non-global LoRA exists, strict isolation runs one same-seed matched baseline pass with the same prompt, projector, spatial guidance, and global LoRAs, but without regional LoRAs. It then restores that baseline latent outside the union of boxes actually targeted by regional LoRAs. The result metadata records the targeted region IDs, latent coverage, extra pass, and maximum outside-latent difference; that difference is expected to be exactly zero. Disable this control only when faster approximate token gating is more important than cross-subject isolation. The extra pass roughly doubles denoising time, but is not run for global-only generations.
...
The prompt-attention field and direct LoRA gate intentionally have different semantics. Prompt placement can feather outside a box so the model composes a coherent scene. LoRA parameter deltas do not use that falloff. With strict isolation disabled, information can still propagate from an enabled lane through normal self-attention even though no LoRA projection is evaluated into an unassigned output lane. With strict isolation enabled, the matched-baseline latent restoration removes that propagated effect outside assigned regional boxes.
