...
The **Projector** tab controls Krea's global 12-column `txtfusion.projector` delta. It provides the `FilterBypass2`, `FilterBypass3`, `skc3vo`, and `z0jglf` reference presets, twelve editable vector fields, and one multiplier that scales the entire vector. The control is off by default and is saved in project JSON and PNG metadata. This is intentionally global: Krea's projector collapses the layerwise text-fusion axis for every sequence position before the single-stream transformer, so trying to confine it to a pixel box would undermine the strict outside-zero regional LoRA gate. K2 Lab applies the projector diff to a cloned generation model before installing regional LoRA hooks, preserving regional LoRA routing unchanged.
